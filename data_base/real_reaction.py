...
def creat_params_1():
    ...
    w_A = round(random.uniform(0.05, 0.5),2)
    w_B = round(random.uniform(0.05, 0.5),2)
    if w_A > w_B:
        w_A, w_B = w_B, w_A
    mole_ratio = round(random.uniform(1.0,5.0),1) # 碳酸氢钠：氯乙醇
...
def creat_params_4(i):
    # 用于创建关于使用WGS制备反应的反应参数（可逆反应）
    # 输出公式
    # 在CSTR反应器中进行WGS反应，反应方程式为CO+H_{2}O \rightleftharpoons H_{2}+C_{2}O，
    # 该反应是可逆反应，用A、B、C、D依次代表四种物质，反应速率方程为r_{A}=k*(p_{A}*p_{B} -p_{C}*p_{D}) ,在1057K，10atm的条件下，
    # k=1345.2 mol/(h \cdot atm \cdot atm),M_A=28 M_B=18 进料摩尔比为1：3，要求氢气H2(C)产量为1000kg/h，假设进料均为纯料，
    # 反应器在恒温恒压条件操作，进料比重假定为3.23 kg/cum
    # 要求一氧化碳CO（A）的转化率达到0.5.求反应器的停留时间和体积。

    # WGS reaction in a CSTR reactor. The reaction equation is CO + H_{2}O \rightleftharpoons H_{2} + CO_{2}.
    # The reaction is reversible. Let A, B, C, D represent the four substances in order. The rate equation is r_{A} = k * (p_{A} * p_{B} - p_{C} * p_{D}).
    # Under conditions of 1057 K and 10 atm,
    # k = 1345.2 mol/(h·atm²), M_A = 28, M_B = 18.
    # The feed molar ratio is 1:3. The required hydrogen H₂ (C) production rate is 1000 kg/h.
    # Assume the feed consists of pure substances.
    # The reactor operates under constant temperature and pressure.
    # The feed density is assumed to be 3.23 kg/m³.
    # The conversion of carbon monoxide CO (A) is required to reach 0.5. Determine the residence time and volume of the reactor.

    param_list = []
    k1 = 1.3452 # kmol/(h \cdot atm \cdot atm)
    kc = 1
    # reactor_idx = 1  # 反应器类型1：BR 2：PFR 3：CSTR
    reactor_idx = random.randint(1,3) # 反应器类型1：BR 2：PFR 3：CSTR
    # m_product = 1100 + 100*i # kg/h
    m_product = random.randint(100,1000)
    mole_ratio = 3 # H2O：CO
    mixraw_density = 3.23 # 比重 kg/m3
    conservation = round(random.uniform(0.2,0.4),2)
    # conservation = 0.45
    p=10 # atm
    param_list.append(reactor_idx)
    param_list.append(m_product)
    ...
    param_list.append(mole_ratio)
    param_list.append(mixraw_density)
    param_list.append(round(conservation,2))
    param_list.append(p)

    return param_list

def solver_4(param_list):
    reactor_idx = param_list[0]
    m_product = param_list[1]
    k1 = param_list[2]
    kc = param_list[3]
    ...
    m_A = n_product*M_A
    m_B = n_product*M_B
    q_raw = (m_A + m_B)/mixraw_density # 原料总体积流量 m3/h
    p_A0 = p/4 # 一氧化碳的初始分压 atm
    p_B0 = p*3/4 # 水整齐的初始分压 atm
    if reactor_idx == 1 or 2:
        def function(x, p_A0, p_B0, k):
            return 1/k*((1-x)*(p_B0-p_A0*x)-p_A0*x*x)
        t,_ = scipy.integrate.quad(function,0,conservation,args=(p_A0,p_B0,k1))
    if reactor_idx == 3:
        t = conservation/k1*((1-conservation)*(p_B0-p_A0*conservation)-p_A0*conservation*conservation)
    v = t * q_raw
    result_list = []
    result_list.append(round(n_product,2))
    result_list.append(round(m_A,2))
    result_list.append(round(m_B,2))
    result_list.append(round(q_raw,2))
    result_list.append(round(p_A0,2))
    result_list.append(round(p_B0,2))
    result_list.append(round(t,3)) # h
    result_list.append(round(v,2))
    return result_list

def dict_maker_4(param_list, result_list):
    reactor_idx = param_list[0]
    reactor_name = REACTION_DICT[reactor_idx]
    m_product = param_list[1]
    k1 = param_list[2]
    ...
    t = result_list[6]
    v = result_list[7]
    # 已知一氧化碳的相对摩尔质量为28 kg/kmol，水的相对摩尔质量为18 kg/kmol，氢气的相对摩尔质量为2 kg/kmol。
    # 每小时需要生产的氢气的摩尔流率为 n_product = {m_product}/2 = {n_product} kmol/h.
    # 根据化学反应方程式可知，每小时所需的一氧化碳的质量流率为 m_A = {n_product}*28 = {m_A} kg/h.
    # 每小时所需的水u的质量流率为 m_B = {n_product}*18 = {m_B} kg/h.
    # 进料的总体积流率为 q_raw = ({m_A}+{m_B})/{mixraw_density} = {q_raw} cum/h.
    # 一氧化碳的初始分压为 p_{A0} = {n_product}*{p}/4 = {p_A0} atm.
    # 水的初始分压为 p_{B0} = {n_product}*{p}*3/4 = {p_B0} atm.
    # 转化率为x时，A的分压为 p_{A} = p_{A0}*(1-x).
    # B的分压为 p_{B} = p_{B0}-p_{A0}*(1-x).
    # C的分压为 p_{C} = p_{A0}*x.
    # D的分压为 p_{D} = p_{A0}*x.
    #
    #
    ...
    # The conversion of carbon monoxide CO (A) is required to reach 0.5. Determine the residence time and volume of the reactor.
    #
    # r"The molar mass of carbon monoxide is 28 kg/kmol, water is 18 kg/kmol, and hydrogen is 2 kg/kmol."
    # r"The required molar flow rate of hydrogen per hour is n_product =  f"{m_product}/2 = {n_product}” kmol/h"
    # f"According to the chemical reaction equation, the required mass flow rate of carbon monoxide per hour is m_A = {n_product}*28 = {m_A} kg/h. "
    # f"The required mass flow rate of water per hour is m_B = {n_product}*18 = {m_B} kg/h. "
    # f"The total volumetric feed flow rate is q_raw = ({m_A}+{m_B})/{mixraw_density} = {q_raw} cum/h. "
    # r"The initial partial pressure of carbon monoxide is p_{A0} = " f"{n_product}*{p}/4 = {p_A0} atm. "
    # r"The initial partial pressure of water is p_{B0} = " f"{n_product}*{p}*3/4 = {p_B0} atm. "
    # r"At conversion x, the partial pressure of A is p_{A} = p_{A0}*(1-x). "
    # r"The partial pressure of B is p_{B} = p_{B0} - p_{A0}*x. "
    # r"The partial pressure of C is p_{C} = p_{A0}*x. "
    # r"The partial pressure of D is p_{D} = p_{A0}*x."
    #
    # f"Theoretical volume is v={t}*{q_raw}={v} l."
    # f"After comprehensively considering various factors in the reaction process, "
    # f"the operating temperatures remain moderate with no presence of highly corrosive substances. "
    # f"Given the cost considerations, we recommend using carbon steel as the reactor material."
    if reactor_idx == 1 or 2:
        t_solve = r"Reaction time obtained through integral calculation: t=\int_{0}^{x_{f} } \frac{p_{A0}dx_{A} }{r_{A}}=\int_{0}^{x_{f} } \frac{p_{A0}dx_{A} }{k1*(p_{A}*p_{B}- \frac{p_{C}*p_{D}}}=\int_{0}^{x_{f} } \frac{dx_{A} }{k1*(1-x_{A})*(p_{B0}-p_{A0}*x_{A})-\frac{p_{A0}*x_{A}*x_{A}}}="f"{t} h. "
    if reactor_idx == 3 :
        t_solve = r"Reaction time obtained through integral calculation: t=\frac{p_{A0} \cdot x_{f}}{r_{A}}=\frac{p_{A0} \cdot x_{f}}{k \cdot p_{A} \cdot p_{B}}=\frac{x_{f}}{k \cdot (1-x_{A}) \cdot (p_{B0}-p_{A0} \cdot x)}="f"{t} h. "
    data_test={ 'prompt':"Please calculate the residence time and reactor volume based on the following conditions, "
                         "and provide recommendations for material selection.",
                'input':f"In a {reactor_name} reactor, "
                        # f"The water-gas shift (WGS) reaction is carried out. "
                        # r"The reaction equation is $\ce{CO + H_{2}O \rightleftharpoons H_{2} + CO_{2}}$."
                        r"The reaction equation is $\ce{A + B \rightleftharpoons C + D}$."
                        r"This reaction is reversible."
                        r"Assigning A, B, C, D to represent the four compounds respectively, "
                        r"the reaction rate equation is $r_{A} = k \left( p_{A} \cdot p_{B} - p_{C} \cdot p_{D} \right)$. "
                        r"At 1057 K and 10 atm, $k = 1345.2 \text{ mol}/(\text{h} \cdot \text{atm}^{2})$, "
                        r"$M_A = 28 \text{ kg/kmol}$, $M_B = 18 \text{ kg/kmol}$, $M_C = 2 \text{ kg/kmol}$. "
                        f"The feed molar ratio of A to B is 1:3. The target hydrogen C production rate is {m_product} kg/h. "
                        f"Assuming pure feed streams, the reactor operates under constant temperature and pressure. "
                        f"The feed density is 3.23 kg/cum. "
                        f"The required conversion of carbon monoxide CO (A) is {conversion_rate}. "
                        f"Please calculate the reactor residence time and volume.",

               'output':
                        # r"The molar mass of carbon monoxide is 28 kg/kmol, water is 18 kg/kmol, and hydrogen is 2 kg/kmol."
                        r"The molar mass of A is 28 kg/kmol, B is 18 kg/kmol, and C is 2 kg/kmol."
                        r"The required molar flow rate of C per hour is n_product = " f"{m_product}/2 = {n_product} kmol/h"
                        f"According to the chemical reaction equation, the required mass flow rate of carbon monoxide per hour is m_A = {n_product}*28 = {m_A} kg/h. "
                        f"The required mass flow rate of water per hour is m_B = {n_product}*18 = {m_B} kg/h. "
                        f"The total volumetric feed flow rate is q_raw = ({m_A}+{m_B})/{mixraw_density} = {q_raw} cum/h. "
                        r"The initial partial pressure of carbon monoxide is p_{A0} = " f"{n_product}*{p}/4 = {p_A0} atm. "
                        ...
                        r"The partial pressure of D is p_{D} = p_{A0}*x."
                        f"{t_solve}"
                        f"Theoretical volume is v={t}*{q_raw}={v} cum."
                        f"After comprehensively considering various factors in the reaction process, "
                        f"the operating temperatures remain moderate with no presence of highly corrosive substances. "
                        f"Given the cost considerations, we recommend using carbon steel as the reactor material."
                    }
...
def main():
    ...
    # with open("data/real_val_test.jsonl","w",encoding="utf-8") as file:
    #     for data in data_list_1:
    #         json_line = json.dumps(data)
    #         file.write(json_line+"\n")
    for i in range(n):
        params_list = creat_params_4(i)
        print("循环次数:",i, params_list)
        result_list = solver_4(params_list)
        print("循环次数:", i, result_list)
        if result_list[6] <= 0 :
            continue
        data_test = dict_maker_4(params_list, result_list)
        data_list_1.append(data_test)
    print(len(data_list_1))
    # with open("data/real_train.jsonl","w",encoding="utf-8") as file:
    #     for data in data_list_1:
    #         json_line = json.dumps(data)
    #         file.write(json_line+"\n")
    save_to = "data/generalization_SBA_test.jsonl"
